import { boolean, char, text, timestamp, uniqueIndex, uuid } from 'drizzle-orm/pg-core';
 ...
 */
export const userRoleAssignments = identitySchema.table(
  ...
  (table) => [
    // One live (active or pending) assignment per user+role.
    uniqueIndex('user_role_assignments_live_unique_idx')
      .on(table.userId, table.roleDefinitionId)
      .where(sql`${table.statusCode} in ('active', 'pending')`),
  ],
);
